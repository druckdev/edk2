...
TPMHelloEntryPoint(
    ...
    //       and inspect
    CopyMem(mPreHashedSHA1   + 1, (UINT8[]) { 0xF4, 0xC0, 0xA7, 0xAA, 0x2A, 0xCE, 0x11, 0xBA, 0xB8, 0x0C, 0x4E, 0x55, 0x5F, 0x9D, 0x63, 0x38, 0x61, 0xED, 0xB4, 0xCE}, mPreHashedSHA1->HashSize);
    CopyMem(mPreHashedSHA256 + 1, (UINT8[]) { 0x87, 0xE7, 0x18, 0xC7, 0x7F, 0xCE, 0xBA, 0x35, 0x6B, 0x89, 0x67, 0x54, 0xBE, 0x4E, 0x03, 0x9F, 0xDE, 0xC9, 0x59, 0xF0, 0x4B, 0x5B, 0x27, 0xD7, 0x12, 0x59, 0xF6, 0x7C, 0x5B, 0x8A, 0xB7, 0x45}, mPreHashedSHA256->HashSize);
    CopyMem(mPreHashedSHA384 + 1, (UINT8[]) { 0x10, 0xEF, 0x98, 0x56, 0x1E, 0x3A, 0xD0, 0xCB, 0xF3, 0xFB, 0x7A, 0x84, 0xD0, 0x1F, 0xBA, 0x54, 0xBD, 0x66, 0x23, 0xD0, 0xC4, 0x40, 0xAB, 0xD2, 0x65, 0x53, 0x75, 0x82, 0xEF, 0xE1, 0xCF, 0x5F, 0xA2, 0x54, 0xAC, 0x11, 0x79, 0xCC, 0x99, 0x0B, 0xC6, 0x75, 0xAA, 0xDA, 0xF9, 0xA9, 0x5C, 0x29}, mPreHashedSHA384->HashSize);
    CopyMem(mPreHashedSHA512 + 1, (UINT8[]) { 0x2E, 0xBB, 0x56, 0xB8, 0x27, 0x72, 0x3D, 0xF2, 0x21, 0x5D, 0x7E, 0x65, 0xE0, 0x75, 0x4D, 0x96, 0xB4, 0x91, 0xA6, 0xAE, 0xF9, 0xFF, 0xBC, 0x45, 0x65, 0x91, 0x6B, 0x42, 0xD5, 0x89, 0xDE, 0xE8, 0x1A, 0xD4, 0x12, 0xFB, 0x98, 0x3F, 0xCB, 0x6B, 0xB2, 0x38, 0xD1, 0x84, 0x6D, 0x38, 0x38, 0xC6, 0xB5, 0xF1, 0x74, 0xCD, 0x14, 0xAA, 0xE3, 0xA3, 0xAE, 0xD2, 0xAD, 0xEC, 0xEB, 0x52, 0x1C, 0x98 }, mPreHashedSHA512->HashSize);

    UINT16 sum = mPreHashedSHA1->HashSize + mPreHashedSHA256->HashSize + mPreHashedSHA384->HashSize + mPreHashedSHA512->HashSize;
    mPreHashedFv = AllocatePool(sizeof(*mPreHashedFv) + sizeof(*mPreHashedSHA1) * 4 + sum);
